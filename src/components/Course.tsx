...
const Course: React.FC<{
  data: TCourse;
  handleQuicklook: (e: React.MouseEvent<HTMLDivElement>) => void;
}> = (props) => {
    ...
    <div className={styles.outer}>
      <Link to={`/detail/${props.data.id}`} className={styles.main}>
        <div
          ...
          id={props.data.id}
          onMouseEnter={(e) => {
            if (e.currentTarget.id === `${props.data.id}`)
              quicklookRef.current?.classList.remove(styles.showQuicklook);
          }}
          onMouseLeave={(e) => {
            if (e.currentTarget.id === `${props.data.id}`)
              quicklookRef.current?.classList.add(styles.showQuicklook);
          }}
        >
          <img
            className={styles.image}
            src={props.data.thumbURL}
            alt="thumbnail"
          ></img>
          <div
            ref={quicklookRef}
            id={props.data.id}
            className={`${styles.quicklook} ${styles.showQuicklook}`}
            onClick={(e) => {
              e.preventDefault();
              props.handleQuicklook(e);
            }}
          >
            Quick Look
          </div>
